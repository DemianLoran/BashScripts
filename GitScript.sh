...
function helpPanel(){
    ...
    echo -e "\n\t${purpleColour}[-st]${endColour}\t\t${yellowColour}Git status directorio actual${endColour}" 
    #Commit con las modificaciones realizadas en el repositorio.
    echo -e "\n\t${purpleColour}[-c][..]${endColour}\t${yellowColour}Commit en el repositorio local ${endColour}${grayColour}Nombre-del-archivo${endColour}"
...
#PROCESO PRINCIPAL
declare -i parameter_counter=0; while getopts 'islastctdvpr' opt; do
    case $opt in
        i) git_Init;;
        s) git_Show;;
        l) git_Log;;
        ...
        st) git_Status;;
        c) argumento1=$2; git_Commit $argumento1;;
